...
// Sheet and header names are part of the downloaded file, so they stay the same in every language.
const rootName = () => fileName?.replace(/\.[^.]+$/, '') || 'Sheet1';
...
function setWorking(working: boolean) { busy = working; convert.disabled = working; if (working) announce(t('jsonExcel.working')); }
...
function renderSheet() {
  ...
  const counts = { rows: counted('tool.rows', data.rows), columns: counted('tool.columns', data.columns), size: formatBytes(blob.size) };
  $('result-summary').textContent = current.sheets.length > 1
    ? t('jsonExcel.summaryMulti', { ...counts, sheets: counted('jsonExcel.sheetCount', current.sheets.length) })
    : t('jsonExcel.summary', counts);
  ...
  const notes: string[] = [];
  if (current.jsonLines) notes.push(t('jsonExcel.jsonLinesNote'));
  if (data.rows > shownRows) notes.push(t('jsonExcel.previewCap', { shown: formatNumber(shownRows), total: formatNumber(data.rows) }));
  const shownColumns = data.preview[0]?.length ?? 0;
  if (data.columns > shownColumns) notes.push(t('jsonExcel.columnsCap', { shown: formatNumber(shownColumns), total: formatNumber(data.columns) }));
  if (current.textNumbers) notes.push(counted('jsonExcel.textNumbers', current.textNumbers));
  if (current.truncated) notes.push(counted('jsonExcel.truncated', current.truncated, { limit: formatNumber(LIMITS.truncated) }));
...
function errorMessage(error: ExcelError) {
  ...
  if ('sheet' in error) {
    if (error.code === 'noData') return t('jsonExcel.noData');
    const key = error.code === 'tooManyColumns' && nested === 'columns' ? 'jsonExcel.tooManyColumnsNumbered' : `jsonExcel.${error.code}`;
    return t(key, { sheet: error.sheet, limit: formatNumber(LIMITS[error.code]) });
  }
