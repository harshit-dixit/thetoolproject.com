...
if (root) {
  ...
  const message = (text: string, error = false) => { status.textContent = text; status.hidden = !text; status.classList.toggle('error', error); };
  const clearResult = () => { result.hidden = true; if (downloadUrl) URL.revokeObjectURL(downloadUrl); downloadUrl = null; };
  const syncMode = () => {
  ...
  const select = (chosen?: File) => {
    if (!chosen) return;
    if (chosen.size > 30 * 1024 * 1024) { message('Choose a PDF no larger than 30MB.', true); return; }
    if (!chosen.name.toLowerCase().endsWith('.pdf') && chosen.type !== 'application/pdf') { message('Choose a PDF file.', true); return; }
    file = chosen;
  ...
  run.addEventListener('click', async () => {
    ...
    try {
      ...
      const output = await compressPdf(file, {
        mode, level: level.value as CompressionLevel, targetBytes: selectedTarget,
        onProgress: message,
      });
      ...
      let note = output.method === 'visual'
        ? 'Pages were rebuilt as images. Selectable text, links, forms and other interactive content are no longer available. Check the downloaded PDF before using it.'
        : output.method === 'optimized'
          ? 'PDF structure was optimized. Check text and interactive elements before use. Existing digital signatures may be invalidated.'
          : 'The original PDF was already the smallest result this tool could produce.';
      if (selectedTarget) note = `${output.reachedTarget ? `At or under ${target.value}KB.` : `Could not reach ${target.value}KB; the actual size is shown above.`} ${note}`;
      get<HTMLElement>('#pdf-result-note').textContent = note;
      result.hidden = false;
      message('');
    } catch (error) {
      message(error instanceof Error ? error.message : 'Could not compress this PDF.', true);
    } finally {
      run.disabled = false;
    }
  });
