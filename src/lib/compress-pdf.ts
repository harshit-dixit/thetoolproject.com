...
export type CompressionErrorCode = 'invalidPdf' | 'passwordOrDamaged' | 'noPages' | 'pageLimit' | 'render' | 'canvas' | 'encode';
export type CompressionProgress = { stage: 'checking' } | { stage: 'page'; current: number; total: number };

/** A failure the page can explain; the caller maps the code to a translated message. */
export class CompressionError extends Error {
  constructor(readonly code: CompressionErrorCode) { super(code); }
}
...
function canvasJpeg(canvas: HTMLCanvasElement, quality: number): Promise<Uint8Array> {
  return new Promise((resolve, reject) => canvas.toBlob(async blob => {
    if (!blob) return reject(new CompressionError('encode'));
...
export async function compressPdf(
  file: File,
  options: { mode: CompressionMode; level: CompressionLevel; targetBytes?: number; onProgress?: (progress: CompressionProgress) => void },
  ...
  if (original.length < 5 || new TextDecoder('ascii').decode(original.subarray(0, 5)) !== '%PDF-') {
    throw new CompressionError('invalidPdf');
  }
  ...
  try {
    ...
    throw new CompressionError('passwordOrDamaged');
  }
  const pages = source.getPageCount();
  if (!pages) throw new CompressionError('noPages');
  if (pages > 60) throw new CompressionError('pageLimit');
  ...
  try {
    ...
    throw new CompressionError('render');
  }
  ...
  try {
    for (const candidate of candidates) {
      ...
      for (let number = 1; number <= pages; number++) {
        onProgress({ stage: 'page', current: number, total: pages });
        ...
        if (!context) throw new CompressionError('canvas');
